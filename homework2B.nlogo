globals [
  ...
  min_distance_in_between
  max_time_out
  search_step
]

turtles-own [intersections]

to setup
  clear-all
  let current_tuttle 0
  set-default-shape turtles "circle"
  ask patches [ set pcolor white ]      ;; plain white background
  set level starting-level
  set min_distance_in_between 1.5
  set max_time_out 1000
  set search_step 2
  setup-level
end

to-report get-current-turtles-position-list
  let position_list []
  foreach sort-on [who] turtles [ the-turtle ->
    ask the-turtle [
      set position_list lput ( list xcor ycor ) position_list
    ]
  ]
  report position_list
end

to put-turtles-at-positions [pos_list]
  let index 0
  foreach sort-on [who] turtles [ the-turtle ->
    ask the-turtle [
      setxy ( item 0 ( item index pos_list ) ) ( item 1 ( item index pos_list ) )
      set index index + 1
    ]
  ]
  refresh-intersections-for-turtles
end

to-report get-operation-steps
  let search_list []
  let time_out 0
  let open []
  let closed []
  ...
  set open lput ( list get-current-turtles-position-list total-intersections -1 ) open
  show open
  while [ solution_found = false and time_out < max_time_out ] [
    ifelse length open != 0 [
      set open sort-by [ [?1 ?2] -> item 2 ?1 < item 2 ?2 ] open

      set current_state item 0 open
      show word "intersections " ( item 2 current_state )
      show word "current state " item 0 current_state
      set open remove-item 0 open
      let parent_index length closed
      set closed lput ( list ( item 0 current_state ) ( item 1 current_state ) ) closed

      let pos_list item 0 current_state
      put-turtles-at-positions pos_list

      ;; check if current_state is already a solution
      ifelse solved? = true [
        set solution_found true
      ]
      [
        let total total-intersections
        ask turtles [
          let x_o xcor
          let y_o ycor
          ;;show word "checking " who

          let reduction_num move-turtle-to-least-intersection who
          let current_position get-current-turtles-position-list

          if ( ( not member-closed current_position closed ) and ( not member-open current_position open ) and ( reduction_num > 0 ) ) [
            set open lput ( list current_position parent_index ( total - reduction_num ) ) open
          ]
    ...
    [
      user-message( "The solution does not exist." )
      report []
    ]

    show word "searched nodes: " time_out
    set time_out time_out + 1
  ]

  ifelse time_out = max_time_out [
    user-message( "Max timeout excceded for this case. Will do another round of search!" )
    report []
  ]
  [
    let all_found false
    while [ all_found = false ] [
      set search_list lput item 0 current_state search_list

      if ( item 1 current_state = 0 ) [
        set all_found true
      ]
      set current_state item ( item 1 current_state ) closed
    ]
    set search_list reverse search_list

    ;; reset position
    put-turtles-at-positions item 0 search_list
  ]

  show word "steps: " ( length search_list )

  report search_list
end

to-report member-open [current_position open]
  let index 0
  let found false
  while [ index < length open ] [
    if ( item 0 ( item index open ) ) = current_position [
      report true
    ]
    set index index + 1
  ]
...
to-report member-closed [current_position closed]
  let index 0
  let found false
  while [ index < length closed ] [
    if ( item 0 ( item index closed ) ) = current_position [
      report true
    ]
    set index index + 1
  ]
...
to setup-level
  ...
  set solution_steps []
  set solution_index 0
  set global_status "setting"
...
to refresh-intersections-for-turtles
  ask turtles [
    set intersections total-intersection-for-a-turtle who
  ]
end

to-report move-turtle-to-least-intersection [index]
  let xcor_i min-pxcor
  let ycor_i min-pycor
  let intersection_num 100000
  let xcor_dst 0
  let ycor_dst 0

  while [xcor_i < max-pxcor] [
    set ycor_i min-pycor
    while [ycor_i < max-pycor] [
      ask turtle index [
        setxy xcor_i ycor_i
        if ( count ( other turtles ) in-radius min_distance_in_between ) = 0 [
          let num total-intersection-for-a-turtle index
          if num < intersection_num [
            set intersection_num num
            set xcor_dst xcor_i
            set ycor_dst ycor_i
          ]
        ]
      ]
      set ycor_i ycor_i + search_step
    ]
    set xcor_i xcor_i + search_step
  ]

  let reduction 0
  ask turtle index [
    setxy xcor_dst ycor_dst
    set reduction intersections - intersection_num
  ]
...
to go
  ...
  if global_status != "done"
  [
    ifelse solution_found = false [
      set global_status "searching"
      set solution_steps get-operation-steps
    ]
    [
      ifelse ( solution_index = length solution_steps and global_status = "showing result" ) [
        ifelse solved? [
          user-message "Done"
          set global_status "done"
        ]
        [
          user-message "Should not hit here"
        ]
      ]
      [
        set global_status "showing result"
        if ( ticks mod 10 = 0 ) [
          put-turtles-at-positions item solution_index solution_steps
          set solution_index solution_index + 1
        ]
      ]
    ]
  ]
...
to-report total-intersections
  let total_num 0
  ask turtles [
    set total_num total_num + total-intersection-for-a-turtle who
  ]
  report (total_num / 2)
end

to-report total-intersection-for-a-turtle [index]
  let total_intersection 0
  ask turtle index [
    ask my-links [
      set total_intersection total_intersection + (intersection-num self links)
    ]
  ]
  report total_intersection
end

to-report intersection-num [link-a link_set]
  let result 0
  ask link_set[
    if crossed? self link-a [
      set result result + 1
    ]
  ]
  report result
